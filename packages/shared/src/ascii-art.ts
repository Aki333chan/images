 ...
 *
 * ПРИЧИНА ОТКАЗА ОСТАЁТСЯ РУССКОЙ намеренно. Единственный, кто её читает, —
 * модель ассистента: проверка вызывается из инструмента send_ascii_art, и
 * отказ уезжает результатом вызова. Дальше модель пересказывает его человеку
 * на языке разговора, как и любой другой ответ инструмента. Переводить строку,
 * которую человек никогда не увидит дословно, незачем.
 *
