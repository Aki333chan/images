...
/**
 * Тюрьма EssentialsX и кто в ней сидит.
 *
 * ПОЧЕМУ ЭТО НЕ ОБЫЧНОЕ БЫСТРОЕ ДЕЙСТВИЕ. Остальные кнопки — это шаблон
 * команды с подстановками: панель ничего не знает о состоянии сервера и не
 * обязана. Тюрьма другая: список тюрем задаёт админ прямо в игре (их бывает
 * и сотня), а команда `togglejail` ведёт себя по-разному в зависимости от
 * того, сидит ли игрок уже и в какой именно тюрьме. Собрать правильную
 * последовательность команд, не зная состояния, нельзя.
 */
export interface MinecraftJailedPlayerDto {
  uuid: string;
  name: string;
  jail: string;
  /**
   * Когда EssentialsX выпустит сам, миллисекунды эпохи. 0 — до отмены,
   * null — неизвестно.
   *
   * Это ЕДИНСТВЕННОЕ время, которое знает сам плагин: момента посадки он не
   * хранит вовсе. «Неизвестно» бывает у тех, кто сидит, но сейчас не в сети:
   * их срок лежит в файле игрока на диске, и плагин его не читает — см.
   * пояснение к online ниже.
   */
  releaseAt: number | null;
  /**
   * Когда посадила панель, миллисекунды эпохи; null — посадили не через неё.
   *
   * Отсюда и берётся «сидит столько-то»: у EssentialsX этого нет, и другого
   * источника не существует. Для посаженных в игре останется только остаток
   * срока — он честнее, чем выдуманное время начала.
   */
  jailedAt: number | null;
  /** Ник сотрудника, посадившего через панель; null — посадили не через неё. */
  jailedBy: string | null;
  /**
   * Игрок сейчас на сервере.
   *
   * Плагин видит только тех, кто в сети: узнать про остальных можно, лишь
   * прочитав файл каждого игрока в userdata, а их бывают десятки тысяч.
   * Офлайн-сидельцы поэтому берутся из записей самой панели, и про них
   * известно ровно то, что она записала: кто и когда посадил.
   */
  online: boolean;
}

export interface MinecraftJailsDto {
  /**
   * EssentialsX отвечает. false — плагина нет или его версия несовместима;
   * это НЕ то же самое, что «тюрем не создано».
   */
  available: boolean;
  /** Настроенные тюрьмы в порядке, в каком их отдаёт плагин. */
  jails: string[];
  jailed: MinecraftJailedPlayerDto[];
}

/** Кого, куда и на сколько. */
export interface MinecraftJailRequestDto {
  player: string;
  jail: string;
  /**
   * Срок в формате EssentialsX: «30m», «2h», «7d», «1mo». Пусто — до отмены.
   *
   * Формат чужой и разбирается на игровом сервере: повторять его разбор в
   * панели значило бы завести второе мнение о том, что такое «1mo».
   */
  duration?: string;
}

/**
 * Сидит ли конкретный игрок — ответ про одного, включая офлайн.
 *
 * Нужен там, где список сидящих принципиально бессилен: он собирается по
 * игрокам в сети, а посадить и выпустить панель должна уметь и того, кого
 * сейчас нет. EssentialsX это позволяет и телепортирует человека в тюрьму
 * при следующем входе.
 */
export interface MinecraftPlayerJailDto {
  /**
   * Сервер знает такого игрока.
   *
   * false — ник с опечаткой либо человек, который ни разу не заходил. Это НЕ
   * «не сидит»: посадить того, кого сервер не знает, всё равно не выйдет.
   */
  known: boolean;
  jailed: boolean;
  /** Где сидит; пусто, если не сидит. */
  jail: string;
  /** Когда выпустят, миллисекунды эпохи; 0 — до отмены. */
  releaseAt: number;
  online: boolean;
}
