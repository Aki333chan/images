 ...
 * админа. Недостающие после обновления ключи берутся из встроенного файла
 * ВЫБРАННОГО языка, поэтому новые сообщения появляются сами, а старые остаются
 * какими были. Русский остаётся последним запасным вариантом лишь для ключа,
 * которого по ошибке нет и во встроенном переводе выбранного языка.
 */
final class LanguageFiles {
    ...
    static Messages load(Plugin plugin, String language) {
        ...
        // Сначала полный встроенный перевод, поверх него — только значения с
        // диска. Так новый ключ доедет при обновлении, а изменённый админом
        // старый текст останется именно его текстом.
        Map<String, Object> chosen = new HashMap<>(fromJar(plugin, language));
        chosen.putAll(fromFile(plugin, language));
