...
/**
 * Быстрые команды — общий слой, а не свойство Paper.
 *
 * Шаблоны здесь ванильные (`say`, `time set day`, `gamemode`), и работают они
 * одинаково на Paper, Forge и NeoForge: это команды самого сервера Minecraft.
 * Поэтому и тесты живут рядом с общей реализацией — чтобы отсюда было видно,
 * что проверяется поведение всего семейства, а не одного модуля.
 */

describe('быстрые команды общего слоя', () => {
  // Никаких зависимостей: сборка команды и фильтр каталога — чистые операции
  // над данными, сети здесь нет.
  const service = new VanillaRconService(
    {} as PrismaService,
