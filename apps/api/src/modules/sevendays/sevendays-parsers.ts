 ...
 *
 * Перечислять known-платформы (Steam, EOS, …) здесь НЕЛЬЗЯ, хотя и хочется:
 * игра уже однажды завела вторую (EOS для кроссплея), заведёт и третью, и
 * тогда запись с незнакомым префиксом тихо пропала бы из списка — а «игрок
 * забанен, но в панели его нет» опаснее лишней строки. Поэтому узнаём по
 * форме: слово, подчёркивание и достаточно длинный хвост. Заголовки таблицы
 * («Id», «Display name», «Reason») под неё не попадают.
 */
const ID_TOKEN = /\b[A-Za-z][A-Za-z0-9]*_[A-Za-z0-9]{6,}\b|\b\d{15,20}\b/;
