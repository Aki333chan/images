 ...
 * одну открытую страницу приходилось бы три запроса вместо одного на каждом
 * тике, а блоки показывали бы состояние с разбегом до интервала опроса.
 *
...
/**
 * Как часто спрашиваем ресурсы у Pterodactyl.
 *
 * Четыре секунды — не «настолько часто, насколько можно», а настолько, чтобы
 * цифры успевали за происходящим. Wings снимает показания раз в секунду и
 * держит последний снимок, так что чаще спрашивать бессмысленно: придёт то
 * же самое. Реже — и всплеск нагрузки на минуту виден с опозданием в десять
 * секунд, из-за чего кнопку «Рестарт» жмут вслепую.
 *
 * Цена — примерно 15 запросов в минуту на одну открытую страницу сервера,
 * и это при том, что на скрытой вкладке опрос останавливается целиком (см.
 * visibilitychange ниже). Клиентский API Pterodactyl по умолчанию разрешает
 * 240 запросов в минуту, то есть запас больше десятикратного.
 *
 * ВАЖНО: это интервал только для показаний Pterodactyl. Всё, что требует
 * похода на сам игровой сервер (TPS по RCON), живёт на своём, куда более
 * редком интервале — см. GAME_POLL_MS.
 */
export const RUNTIME_POLL_MS = 4_000;

/**
 * Как часто спрашиваем сам игровой сервер (TPS, MSPT).
 *
 * Намеренно отдельно от RUNTIME_POLL_MS и намеренно реже. За этими цифрами
 * стоит не снимок, который панель Pterodactyl уже держит наготове, а живая
 * RCON-команда: сервер её выполняет в главном потоке, между тиками игры.
 * Ускорять её заодно с полосой ресурсов значило бы дёргать игровой сервер
 * втрое чаще ради показателя, который и меняется-то медленно.
 */
 ...
 * либо вечно устаревший список плагинов, либо его перечитывание на каждом
 * опросе, и заметить это на глаз тяжело.
 */
 ...
 *
 * Нужно после нажатия «Старт» или «Стоп»: ждать даже один тик, чтобы значок
 * наконец сменился, — это ровно то ощущение «кнопка не сработала», из-за
 * которого её жмут второй раз.
 */
