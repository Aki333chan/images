...
/**
 * Текст, пришедший от API.
 *
 * Сервер шлёт либо ключ словаря, либо чужую строку: сообщение плагина, ответ
 * игрового сервера, отказ провайдера экономики. Отличаются они ровно так же,
 * как их отличает фильтр ошибок на бэкенде, — ключ это то, что в словаре
 * есть. Чужая строка проходит насквозь и показывается как есть: вывод
 * плагина панель не переводит, он на своём языке и должен таким остаться.
 *
 * Пустое значение даёт пустую строку, а не «undefined» на экране.
 */
export function useApiText(): (text: string | null | undefined, values?: Values) => string {
  const t = useT();
  return (text, values) => (text ? t(text, values) : '');
}
