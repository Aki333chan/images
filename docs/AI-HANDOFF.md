...
**Безопасный контракт абсолютного `set` баланса.**

Idempotency произвольных команд оферт закрыта в AddonsNPC 2.1.0 двумя режимами.
Plain/`once:` гарантирует at-most-once, фиксируя cursor до dispatch; потенциальный пропуск
в аварийном окне честно документирован. `idempotent:` требует `{idempotency_key}` и
разрешает retry только для получателя, который хранит и дедуплицирует этот ключ. Старые
строки автоматически получают безопасный plain-режим, payload schema 1 читается без
миграции. Бесплатные предложения теперь также проходят через claim.

Следующая разработка — решить, нужен ли абсолютный `set` через Companion/панель, и если
нужен, реализовать его отдельной операцией с expected balance/revision и receipt. Нельзя
эмулировать его чтением текущего баланса плюс delta-transfer: конкурентная проводка между
чтением и записью изменила бы не тот итог.
...
AddonsNPC 2.1.0, AurumGuilds 0.4.0, AurumArena 1.5.0, AurumCompanion 0.11.0,
AurumUI 0.7.0.
...
`docs/aurum-plugin-compatibility.md`. Редактор и command-idempotency после этого закрыты;
следующий этап — безопасная семантика абсолютного `set`.

## Очередь после текущего этапа

1. Отдельный безопасный контракт абсолютного `set` баланса, если он действительно нужен.
2. Опциональная миграция динамических настроек.
3. Полный staging Paper 26.2 + MariaDB + VaultUnlocked, fault injection и Spark.
