...
**Idempotency-контракт для произвольных команд оферт.**

Редактор правил завершён в Core 0.15.0 + Companion 0.10.0. `AurumRulesAdminApi`
работает поверх существующих revision repositories: preview принимает точную полную
замену, проверяет expected revision и выдаёт actor-bound одноразовый токен на пять минут;
apply требует причину и атомарно сохраняет следующую immutable-ревизию. Веб-панель не
пишет YAML/SQL и не отправляет произвольные команды. Конфликт возвращает актуальную запись.
`APPLIED_RELOAD_FAILED` является успешным DB commit и никогда не повторяется автоматически.

Следующая разработка — определить receipt/idempotency для произвольных команд, которые
исполняются как часть оферт. Встроенные предметные операции уже защищены PDC receipt, но
произвольная команда может выполниться, а её MariaDB cursor не успеть продвинуться до
аварии. Нужен явный контракт допустимых исполнителей и восстановления; нельзя обещать
exactly-once любой внешней команде, если принимающий плагин не поддерживает ключ операции.
...
`docs/aurum-plugin-compatibility.md`. Редактор после этого уточнения закрыт; следующий
этап — idempotency произвольных команд оферт.

## Очередь после текущего этапа

1. Контракт idempotency произвольных команд оферт.
2. Отдельный безопасный контракт абсолютного `set` баланса, если он действительно нужен.
3. Опциональная миграция динамических настроек.
