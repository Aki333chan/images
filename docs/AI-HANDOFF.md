...
**Решение по опциональной миграции динамических настроек.**
...
Следующая работа — коротко инвентаризировать динамические YAML-настройки Arena/NPC/Slots и
связи Guilds, затем выбрать только действительно нужный перенос. Пользователь подтвердил,
что сервер тестовый и NPC/арены можно пересоздать, поэтому не строить тяжёлую универсальную
миграцию ради тестовых данных. Базовые совместимые поля можно сохранить дешёвыми
одноразовыми импортерами; всё остальное явно оставить на пересоздание перед staging.
...
1. Опциональная минимальная миграция динамических настроек либо документированное
   пересоздание там, где перенос не окупается.
