...
- Trade: окна нет, сделка ведётся командами. Логика от этого не страдает — она в
  ревизиях, — но закрыть окно и тем отменить сделку пока нельзя, только `/trade cancel`.
- Trade: предмет, выложенный на стол, не существует физически нигде — только записью в
  MariaDB. Это осознанно: две копии, которые могут разойтись, — это две копии, которые
  могут задвоиться. Следствие: посмотреть выложенное можно только через `/trade view`.
...
**GUI одной сделки. Сама сделка работает командами; окна нет.**

AurumCore 0.9.0 сделал гарантированный обмен целиком, кроме окна: ревизии и
подтверждения, изъятие предметов в запись, расчёт денег через holds, возврат и выдачу
заявками, команды `/trade`. Включается `trading.enabled` (по умолчанию выключено).

Окно делается ОБОЛОЧКОЙ над `TradeCoordinator`, а не вторым местом, где записаны
правила. Что оно обязано уважать:

1. предмет уходит из инвентаря в момент выкладывания, а не при расчёте;
2. любое изменение стола обнуляет оба подтверждения — кнопка подтверждения должна
   отправлять ревизию, которую игрок видел, и получать отказ, если она устарела;
3. закрытие окна — это `/trade cancel`, а не молчаливый выход: на столе чужие вещи.
...
1. GUI сделки поверх готового сервиса.
2. Экономические экраны AurumUI.
