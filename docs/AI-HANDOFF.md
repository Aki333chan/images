...
**Гарантированная торговля между игроками. Работа не начиналась.**

Все потребители AddonsNPC переведены на заявки: магазины, скупщики и гильдейские торговцы.
`transactions.yml` больше не пополняется и удаляется сам, когда дочищен; на новой установке
не создаётся. `NpcSaga`, `NpcSagaRepository` и `EconomyService.recover` остались ТОЛЬКО
ради операций, начатых 1.9.0 до обновления, и удаляются в 2.1.0, когда обновление точно
везде прошло.

Торговля — первый потребитель заявок, где предметы идут в ОБЕ стороны, и это новое:

1. GUI сделки с повторным подтверждением обеих сторон после любого изменения;
2. `aurum_trades` и `aurum_trade_offers` в Core лежат с migration 1 и до сих пор не
   используются ни одной строчкой кода — решить, брать их или вести сделку заявками;
3. две заявки (по одной на сторону) против одной с шагами на обоих — вопрос открытый:
   одна заявка проще, но её шаги должны уметь трогать инвентари двух игроков, из которых
   один может выйти из игры посреди доставки.
...
1. Гарантированная торговля между игроками.
