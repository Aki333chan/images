...
- NPC: магазины переведены на claims, СКУПЩИКИ И ГИЛЬДЕЙСКИЕ ТОРГОВЦЫ — ещё нет. У них
  остаётся прежнее окно, причём у скупщика оно теряет предметы игрока, а не деньги сервера.
...
**Скупщики и гильдейские торговцы на claims. Магазины уже переведены, эти два — нет.**

AddonsNPC 2.0.0 перевёл на заявки покупки в магазинах: деньги, запись долга, выдача.
Окно «предметы выданы, журнал ещё HELD» закрыто, пост-команды больше не теряются.

Осталось зеркальное направление:

1. **Скупщик** (`BuyerService`): предметы игрока забираются, затем выплачиваются деньги.
   Авария между шагами теряет ПРЕДМЕТЫ ИГРОКА. Лечится тем же приёмом, шаги другие:
   шаг 0 — забрать предметы, шаг 1 — списать зарезервированные деньги на игрока.
2. **Гильдейский торговец** (`GuildTraderService`): выдача бонуса и capture. Сейчас
   восстановление опознаёт выданный бонус по `guild-actor`; на заявках это станет
   обычным курсором шагов.
3. После этого `transactions.yml` можно убрать целиком вместе с `NpcSaga`.
...
1. Скупщики и гильдейские торговцы AddonsNPC на claims.
