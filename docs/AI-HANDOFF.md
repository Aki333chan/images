...
Единственное состояние, которое обязательно переносится, — реальные балансы текущего
Vault provider. Использовать уже готовую последовательность Core passive → shadow
snapshot/import/verify → backup/rollback export → active cutover. Нельзя ставить Core сразу
в active и нельзя заранее отключать Essentials economy provider. Перед переходом сохранить
старые каталоги всех плагинов; если там неожиданно окажутся ценные Guilds/Slots данные,
оценить их отдельно до удаления, не возвращаясь к универсальной миграции.
...
1. Точный runbook первого развёртывания: backup, состав пакета, конфиги, Vault shadow-import,
   проверка, active cutover, обновление панели и rollback.
...
- Сохраняем только реальные деньги через штатный shadow snapshot/import/verify Core.
