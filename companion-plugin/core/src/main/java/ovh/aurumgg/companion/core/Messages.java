...
public final class Messages {
    ...
    public String get(String key, Map<String, String> values) {
        ...
        return apply(String.valueOf(raw), values);
    }
    ...
    public List<String> list(String key, Map<String, String> values) {
        ...
        if (raw instanceof List<?> list) {
            return list.stream().map(item -> apply(String.valueOf(item), values)).toList();
        }
        if (raw == null) return List.of(key);
        return List.of(apply(String.valueOf(raw), values));
    }
    ...
    /**
     * Подстановка значений вида {name}.
     *
     ...
     *
     * Неизвестный плейсхолдер остаётся как есть: так опечатку в файле языка
     * видно сразу, а не «пропало слово».
     */
