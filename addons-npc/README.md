...
- редактируемые `config.yml`, `locales/*.yml`, `npcs.yml`, `shops.yml`, `buyers.yml` и `guild-traders.yml` с горячей перезагрузкой.
...
Скопируйте `target/AddonsNPC-1.7.0.jar` в `plugins/`, перезапустите сервер и
