...
- Java 25+;
- AurumCore 0.7.0+ — обязательная зависимость; платная торговля использует native holds API.
...
Скопируйте `target/AddonsNPC-1.9.0.jar` в `plugins/` рядом с AurumCore 0.7.0+, перезапустите сервер и
выполните `/npc help`. Для сборки нужен JDK 25.

Незавершённые операции автоматически записываются в `plugins/AddonsNPC/transactions.yml`.
Это служебный журнал: не редактируйте и не удаляйте его во время работы сервера. Плагин
периодически завершает применённые операции либо освобождает неиспользованные резервы.
