...
public final class NpcCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try {
            ...
            messages.localizedRaw(sender, messages.prefix() + "&c" + exception.getMessage(), Map.of());
            return true;
        }
    }
    ...
    private boolean list(CommandSender sender) {
        ...
        messages.localizedRaw(sender, messages.prefix() + "&6NPCs (&e" + npcs.ids().size() + "&6): " + ids, Map.of());
        return true;
    }

    private boolean info(CommandSender sender, String[] args) {
        NpcDefinition npc = requireNpc(args);
        messages.localizedRaw(sender, "&6--- NPC " + npc.id() + " ---", Map.of());
        messages.localizedRaw(sender, "&7Name: &f" + npc.name() + " &7Type: &f" + npc.entityType()
                + " &7Enabled: &f" + npc.enabled(), Map.of());
        messages.localizedRaw(sender, "&7World: &f" + npc.location().world() + " &7XYZ: &f"
                + round(npc.location().x()) + ", " + round(npc.location().y()) + ", " + round(npc.location().z()), Map.of());
        messages.localizedRaw(sender, "&7Skin: &f" + npc.skin().type() + " " + npc.skin().value(), Map.of());
        messages.localizedRaw(sender, "&7Click: &f" + npc.clickMode() + " &7Cooldown: &f" + npc.cooldownSeconds()
                + "s &7Permission: &f" + (npc.permission().isBlank() ? "none" : npc.permission()), Map.of());
        messages.localizedRaw(sender, "&7Visibility: &f" + npc.visibilityRange() + " &7Name: &f" + npc.nameVisibilityRange()
                + " &7Look: &f" + npc.lookMode() + " " + npc.lookRange(), Map.of());
        messages.localizedRaw(sender, "&7Dialogue: &f" + npc.dialogueMode() + " (&e" + npc.messages().size()
                + "&f lines) &7Actions: &f" + npc.actions().size(), Map.of());
        messages.localizedRaw(sender, "&7Hands: &fR=" + itemName(npc.rightHand()) + " &7L=&f" + itemName(npc.leftHand())
    ...
    private boolean dialogue(CommandSender sender, String[] args) {
        ...
        switch (operation) {
            ...
            case "list" -> {
                messages.localizedRaw(sender, "&6Dialogue lines for " + npc.id() + ":", Map.of());
    ...
    private boolean action(CommandSender sender, String[] args) {
        ...
        switch (operation) {
            ...
            case "list" -> {
                messages.localizedRaw(sender, "&6Actions for " + npc.id() + ":", Map.of());
    ...
    private boolean shop(CommandSender sender, String[] args) {
        ...
        return switch (args[1].toLowerCase(Locale.ROOT)) {
            case "list" -> {
                messages.localizedRaw(sender, messages.prefix() + "&6Shops: &e" + String.join("&7, &e", shops.ids()), Map.of());
                yield true;
            }
            case "create" -> {
    ...
    private boolean editBuyerLines(CommandSender sender, List<String> lines, String[] args, String type) {
        switch (args[4].toLowerCase(Locale.ROOT)) {
            ...
            case "list" -> {
                messages.localizedRaw(sender, "&6Buyer " + type + " entries:", Map.of());
    ...
    private boolean guildTrader(CommandSender sender, String[] args) {
        ...
        return switch (args[1].toLowerCase(Locale.ROOT)) {
            case "list" -> {
                messages.localizedRaw(sender, messages.prefix() + "&6Guild traders: &e"
            ...
            case "create" -> {
                ...
                guildTraders.put(new GuildTraderDefinition(id,
                        args.length > 3 ? join(args, 3) : messages.text("gui.guild-trader.default-title"), 27));
    ...
    private boolean guildTraderLore(CommandSender sender, String[] args) {
        ...
        switch (args[4].toLowerCase(Locale.ROOT)) {
            ...
            case "list" -> {
                messages.localizedRaw(sender, "&6Guild bonus lore:", Map.of());
    ...
    private void help(CommandSender sender) {
        for (String line : messages.lines("command-help")) messages.raw(sender, line, Map.of());
    }
    ...
    private void ok(CommandSender sender, String text) {
        messages.localizedRaw(sender, messages.prefix() + "&a" + text, Map.of());
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (root.equals("action") && args.length == 5 && args[1].equalsIgnoreCase("add")) {
            return switch (args[3].toLowerCase(Locale.ROOT)) {
                ...
                case "message" -> filter(messages.lines("tab.message-examples"), args[4]);
                default -> List.of();
            };
        }
        ...
        if (root.equals("buyer")) {
            ...
            if (operation.equals("lore") && args.length == 6 && args[4].equalsIgnoreCase("add"))
                return filter(messages.lines("tab.buyer-lore-examples"), args[5]);
            if (operation.equals("command") && args.length == 6 && args[4].equalsIgnoreCase("add"))
                return filter(messages.lines("tab.buyer-command-examples"), args[5]);
        }
