  ...
  supplies:
    ...
    offers:
      11:
        ...
        permission: ""
        commands:
          - "say {player} купил хлеб" # по умолчанию at-most-once
          - "idempotent:rewards grant {player} vip {idempotency_key}"
        discount:
...
конечный склад на это число. `price: 0` создаёт бесплатное предложение и не требует hold.
...
У пост-команд есть два аварийных режима:

- строка без префикса либо `once:<command>` — at-most-once: Core сначала фиксирует
  курсор, затем команда запускается. Дубликата не будет, но авария в этом коротком окне
  может пропустить команду;
- `idempotent:<command> ... {idempotency_key}` — повторяемая доставка. AddonsNPC
  подставляет уникальный стабильный ключ конкретной покупки и шага. Получатель команды
  обязан хранить его и возвращать уже записанный результат при повторе.

`idempotent:` без `{idempotency_key}` отклоняется. Не ставьте этот префикс перед `/give`
или другой командой, которая не умеет дедуплицировать ключ: exactly-once нельзя создать
одной строкой конфигурации. Бесплатные предложения тоже записываются как claim и используют
тот же контракт, но не имеют денежного шага.
    ...
    offers:
      11:
        ...
        commands:
          ...
          - "idempotent:rewards sale {player} {idempotency_key}"
        bonus:
...
после полной проверки сделки. Выплата резервируется в AurumCore до изъятия предметов;
при окончательном отказе изъятые стаки возвращаются без перезаписи остального инвентаря.
...
`{bonus_remaining}`. Продажа требует active AurumCore 0.7.0+ и permission `addonsnpc.buyer`;
...
Платные предложения требуют active AurumCore 0.7.0+. Сначала создаётся hold, затем асинхронно
вызывается `grantBonus`, и только после успешной выдачи платёж фиксируется. Уникальный actor
позволяет после рестарта определить, был ли бонус уже применён. Пока операция не завершилась,
повторная покупка тем же игроком заблокирована.
...
AddonsNPC 1.9.0 требует AurumCore 0.7.0+. Обменники используют atomic exchange API,
а магазины, скупщики и гильдейские торговцы — durable holds. `transactions.yml` является
автоматически обслуживаемым WAL незавершённых операций. В `config.yml` настраиваются
`economy.hold-ttl-seconds` (10–3600) и `economy.recovery-retry-seconds`.
...
Paper и Spigot 26.3 предоставляют разные сигнатуры API профиля `Mannequin`. Плагин определяет
